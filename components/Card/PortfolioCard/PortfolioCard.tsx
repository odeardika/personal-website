...
export default function PortfolioCard({ props, index }: { props: PortfolioCardProps, index: number }) {
  ...
  return (
    <div className={`bg-white rounded-[10px] shadow-xl w-100 hover:cursor-pointer hover:scale-110 transition-transform duration-300 flex flex-col justify-between ${originPosition[index]}`}>
      ...
      <div className='px-4 pb-4 flex flex-col flex-grow justify-between'>
        <div>
          <h3 className="text-2xl my-2 font-semibold text-dark_blue ">{props.project_title}</h3>
        ...
        <div className='flex gap-2'>
          ...
          <Link href={props.github_url? props.github_url : ""} target="_blank" rel="noopener noreferrer" className='flex p-2 border-sky-500 text-sky-500 hover:text-white hover:bg-sky-500 hover:border-sky-500 hover:shadow-lg hover:shadow-sky-500/50 border-2 rounded-md items-center gap-1 hover:-translate-y-2 transition-transform duration-300 '>
